...
class AOE : public LogicComponent
{
	OBJECT(AOE);
public:
	AOE(Context* context, Urho3DPlayer* main);
	~AOE();
	virtual void Start();
	...
	void StartAOE(Vector3 targetPos, float timeRamp, bool sendToServer);
	...
	bool silence_;
	float AOEDuration_;
	float AOEElapsedTime_;
	float AOEInterval_;
	float AOEIntervalElapsedTime_;
	bool AOEed_;
	int targetClientID_;
