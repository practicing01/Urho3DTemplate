...
class Crit : public LogicComponent
{
	OBJECT(Crit);
public:
	Crit(Context* context, Urho3DPlayer* main);
	~Crit();
	virtual void Start();
	...
	void StartCrit(Vector3 pos, bool sendToServer);
	...
	void HandleSetHealth(StringHash eventType, VariantMap& eventData);
