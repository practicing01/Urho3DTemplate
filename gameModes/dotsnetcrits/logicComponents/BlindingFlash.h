...
class BlindingFlash : public LogicComponent
{
	OBJECT(BlindingFlash);
public:
	BlindingFlash(Context* context, Urho3DPlayer* main);
	~BlindingFlash();
	virtual void Start();
	...
	void StartBlindingFlash(int clientID, float timeRamp, bool sendToServer);
	...
	float cooldown_;
	bool silence_;
	float BlindingFlashDuration_;
	float BlindingFlashElapsedTime_;
	bool BlindingFlashed_;
	int targetClientID_;
